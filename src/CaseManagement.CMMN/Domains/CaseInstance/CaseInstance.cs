...
namespace CaseManagement.CMMN.Domains
{
    ...
    public class CaseInstance : BaseAggregate
    {
        ...
        public CaseInstance(string id, DateTime createDateTime, string workflowDefinitionId) : base()
        {
            ...
            CaseDefinitionId = workflowDefinitionId;
        }
        ...
        public ICollection<Criteria> GetEntryCriterions(string id, CaseDefinition workflowDefinition)
        {
        ...
        public bool IsRepetitionRuleSatisfied(string planItemId, CaseDefinition workflowDefinition, bool listenEvent = false)
        {
        ...
        public bool IsManualActivationRuleSatisfied(string id, CaseDefinition workflowDefinition)
        {
        ...
        public void CreateFormInstance(string elementId, string formId, string performerRef)
        {
            lock (DomainEvents)
            {
                var evt = new CaseElementInstanceFormCreatedEvent(Guid.NewGuid().ToString(), Id, Version + 1, elementId, Guid.NewGuid().ToString(), formId, performerRef, DateTime.UtcNow);
                Handle(evt);
                DomainEvents.Add(evt);
            }
        }
        ...
        public static CaseInstance New(CaseDefinition workflowDefinition)
        {
        ...
        public static CaseInstance New(List<DomainEvent> evts)
        {
            var result = new CaseInstance();
        ...
        public override void Handle(object obj)
        {
            ...
            if (obj is CaseElementInstanceFormCreatedEvent)
            {
                Handle((CaseElementInstanceFormCreatedEvent)obj);
            }

            if (obj is CaseElementInstanceFormSubmittedEvent)
            {
                Handle((CaseElementInstanceFormSubmittedEvent)obj);
            }
        ...
        private void Handle(CaseInstanceCreatedEvent evt)
        {
            ...
            CaseDefinitionId = evt.CaseDefinitionId;
        }
        ...
        private void Handle(CaseElementInstanceFormCreatedEvent evt)
        {
            var planItemInstance = WorkflowElementInstances.First(p => p.Id == evt.CaseElementId);
            planItemInstance.FormInstanceId = evt.FormInstanceId;
            Version++;
            RaiseEvent(evt);
        }

        private void Handle(CaseElementInstanceFormSubmittedEvent evt)
        {
            Version++;
            RaiseEvent(evt);
        }
        ...
        {
            var workflowInstance = obj as CaseInstance;
        ...
        {
            return new CaseInstance(Id, CreateDateTime, CaseDefinitionId)
            {
