...
namespace CaseManagement.CMMN.Persistence.InMemory
{
    public class InMemoryStatisticQueryRepository : IStatisticQueryRepository
    {
        ...
        public InMemoryStatisticQueryRepository(ConcurrentBag<CaseDefinitionStatisticAggregate> workflowDefinitionStatistics, ConcurrentBag<DailyStatisticAggregate> caseDailyStatistics)
        {
            _workflowDefinitionStatistics = workflowDefinitionStatistics;
            _caseDailyStatistics = caseDailyStatistics;
        }

        public Task<CaseDefinitionStatisticAggregate> FindById(string id)
        {
            return Task.FromResult(_workflowDefinitionStatistics.FirstOrDefault(w => w.CaseDefinitionId == id));
        }
