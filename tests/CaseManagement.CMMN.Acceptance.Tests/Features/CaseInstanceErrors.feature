...
Scenario: Check errors are returned when user is not authorized to submit the form
	When execute HTTP POST JSON request 'http://localhost/case-instances'
	| Key                | Value					|
...
Scenario: Check errors are returned when activating element and case-instance doesn't exist
	When execute HTTP GET request 'http://localhost/case-instances/unknown/activate/PI_ProcessTask_1'
	And extract JSON from body
	
	Then HTTP status code equals to '404'
	Then JSON 'errors.bad_request[0]'='case instance doesn't exist'

Scenario: Check errors are returned when activating element and case-instance-element doesn't exist
	When execute HTTP POST JSON request 'http://localhost/case-instances'
	| Key                | Value					|
	...
	And extract 'id' from JSON body
	And wait '5' seconds
	And execute HTTP GET request 'http://localhost/case-instances/$id$/activate/unknown'
	And extract JSON from body
