...
Scenario: Launch caseWithTwoStages and check his status is completed
	...
	And extract 'id' from JSON body into 'insid'
	And execute HTTP GET request 'http://localhost/case-plan-instances/$insid$/launch'		
	And poll 'http://localhost/case-plan-instances/$insid$', until '$.elements[?(@.definition_id == 'PlanItem_1291396')].state'='Failed'
	And poll 'http://localhost/case-plan-instances/$insid$', until '$.elements[?(@.definition_id == 'PlanItem_0ox3upg')].state'='Failed'
	And poll 'http://localhost/case-plan-instances/$insid$', until '$.elements[?(@.definition_id == 'PlanItem_0ctfxhw')].state'='Completed'
	And poll 'http://localhost/case-plan-instances/$insid$', until '$.elements[?(@.definition_id == 'PlanItem_1fd01pl')].state'='Completed'
...
Scenario: Launch caseWithOneCaseFileItem and check his status is completed
	...
	And poll 'http://localhost/case-plan-instances/$insid$', until '$.elements[?(@.definition_id == 'PlanItem_0uxcsmm')].state'='Completed'
	And execute HTTP GET request 'http://localhost/case-plan-instances/$insid$/terminate'
	And poll 'http://localhost/case-plan-instances/$insid$', until 'state'='Terminated'
	And extract JSON from body
...
Scenario: Launch caseWithOneBlockedSEntry and check his status is active
	...
	And extract 'id' from JSON body into 'insid'
	And execute HTTP GET request 'http://localhost/case-plan-instances/$insid$/launch'	
	And poll 'http://localhost/case-plan-instances/$insid$', until '$.elements[?(@.definition_id == 'PlanItem_18sdiyp')].state'='Completed'
	And extract JSON from body
	And execute HTTP GET request 'http://localhost/case-plan-instances/$insid$/terminate'
	And poll 'http://localhost/case-plan-instances/$insid$', until 'state'='Terminated'
...
Scenario: Reactive failed caseWithTwoStages and check his status is failed (scope = Stage)
	...
	And poll 'http://localhost/case-plan-instances/$insid$', until '$.elements[?(@.definition_id == 'PlanItem_1291396')].state_histories[4].state'='Failed'
	And execute HTTP GET request 'http://localhost/case-plan-instances/$insid$/terminate'
	And poll 'http://localhost/case-plan-instances/$insid$', until 'state'='Terminated'
	And extract JSON from body
